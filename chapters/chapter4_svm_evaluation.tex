...
Greedy forward and backward selection methods were removed from the analysis
pipeline and codebase. They required orders of magnitude more model fits than
other approaches (forward selection incurred millions of SVM fits per run) and
delivered unstable or inferior predictive performance on high-dimensional
genomic data. The report therefore focuses on the retained, practical methods
(ANOVA pre-filtering, SVM-RFE with pre-filtering, and cross-validated LASSO),
which give better computational and predictive trade-offs for this project.
...
\subsection*{Prefiltering Policy (summary)}

The wrapper methods used by the pipeline do not run on the full 54{,}675
probe space directly. Instead, an ANOVA-based prefilter is applied inside
each training fold to reduce the candidate pool before any wrapper algorithm
executes. The policy is:

\begin{itemize}
    \item Compute ANOVA F-scores and p-values on the training fold.
    \item If a p-value threshold $\alpha$ is supplied, retain features with
        p~$\leq \alpha$; otherwise retain the top-$M$ probes by F-score.
    \item The default cap is $M=500$; wrapper algorithms operate on this
        prefiltered subset to keep runtime and memory within practical bounds.
\end{itemize}

This prefiltering is used by SVM-RFE and the RandomForest-importance selector
and is explicitly documented in the methods' descriptions earlier in this chapter.

\paragraph{Why we prefilter}
The prefilter step is intentionally conservative and serves three goals: (1)
it makes wrapper methods computationally tractable on tens of thousands of
probes, (2) it improves the robustness of importance/ranking estimates by
removing features with weak univariate signal, and (3) it reduces the risk of
overfitting during multivariate wrapper searches. To preserve honest
evaluation, the ANOVA prefilter is computed using only training-fold data so
that no information from held-out test folds influences the candidate pool.
 
\paragraph{Parallel execution note}
The `svm_classifier` script offers an optional parallel mode for Path B
evaluations (via the `--max-workers` flag). Parallelising methods reduces
wall-clock time when multiple CPU cores are available but may change the
ordering and timing of randomised steps (e.g. RF training). For reproducible
results, set `--max-workers 1` (the default) or fix the random seed across
runs. Logs and per-method results are still written to the same results
directory so parallel runs produce the same artifacts — errors are reported
per-method in the main log.
\subsection*{Prefiltering: runtime estimate}

The number of SVM fits required by greedy RFE per cross-validation fold can be
written as
$$
\text{Models per fold} = \sum_{i=0}^{k-1}(N - i) = kN - \frac{k(k-1)}{2},
$$
where $N$ is the number of candidate features and $k$ the target subset size.
For the full microarray ($N=54{,}675$) and $k=20$ this yields
$$
\text{Models}_{\text{full}} = 20\times54{,}675 - \frac{20\times19}{2} = 1,093,410.
$$
With a conservative ANOVA prefilter capped at $M=500$ the same calculation
gives
$$
\text{Models}_{\text{prefilter}} = 20\times500 - \frac{20\times19}{2} = 9,810.
$$
Thus the prefilter reduces the number of model fits by a factor of
approximately $1{,}093{,}410 / 9{,}810 \approx 111.5$ (over two orders of
magnitude). Even if the per-fit time is small, this reduction makes wrapper
searches feasible on commodity hardware; measured wall-clock speedups depend
on implementation details and the chosen estimator but the model-fit count
provides a reliable worst-case scaling estimate.
